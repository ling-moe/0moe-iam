...
import cn.lingmoe.iam.infra.mapper.SysPostMapper;
import cn.lingmoe.iam.infra.mapper.SysUserPostMapper;
...
public class SysPostServiceImpl implements ISysPostService {
     ...
     */
    @Override
    public int deletePostByIds(String ids) throws CommonException {
        Long[] postIds = Convert.toLongArray(ids);
        for (Long postId : postIds) {
            SysPost post = selectPostById(postId);
            if (countUserPostById(postId) > 0) {
                throw new CommonException(String.format("%1$s已分配,不能删除", post.getPostName()));
            }
        }
